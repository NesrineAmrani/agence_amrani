        ...
        <div class="w-full px-4 overflow-auto">
            ...
            <div class="flex items-center justify-between pt-6 gap-4">
                <div class="flex items-center gap-1 lg:gap-4">
                    ...
                    <select class="form-input w-48" id="villa_service_id">
                        <option value="-1">Services</option>
                        @foreach ($services as $service)
                        <option value="{{$service->id}}">{{$service->villa_service}}</option>
                        @endforeach
                    </select>
                    <select class="form-input w-44" id="villa_situation">
                ...
                <a href="{{ route('villa.create') }}" class="border px-4 py-2 rounded-lg bg-blue-400 hover:bg-gray-400 text-white text-sm">
                ...
                <table class="min-w-max w-full table-auto">
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        @foreach ($villas as $villa)
                            @include('amrani.pages.villa.partials.tr', ['villa'=>$villa])
                        @endforeach
                    </tbody>
                </table>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.destroy_villa').on('click', function(e){
            e.preventDefault();
            var that = $(this);
            Swal.fire({
                title: 'Supprimer?',
        ...
        $('#req_submit').on('click', function(){
            ...
            if($('#villa_service_id').val() != "-1"){
                data.villa_service_id = $('#villa_service_id').val();
            }
            if($('#villa_situation').val() != "-1"){
                data.villa_situation = $('#villa_situation').val();
            }
