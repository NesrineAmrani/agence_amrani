        ...
        <div class="w-full px-4 overflow-auto">
            ...
            <div class="flex items-center justify-between pt-6 gap-4">
                <div class="flex items-center gap-1 lg:gap-4">
                    ...
                    <select class="form-input w-48" id="ferma_service_id">
                        <option value="-1">Services</option>
                        @foreach ($services as $service)
                        <option value="{{$service->id}}">{{$service->ferma_service}}</option>
                        @endforeach
                    </select>
                    <select class="form-input w-44" id="ferma_situation">
                ...
                <a href="{{ route('ferma.create') }}" class="border px-4 py-2 rounded-lg bg-blue-400 hover:bg-gray-400 text-white text-sm">
            ...
            <div class="bg-white shadow-md rounded my-6">
                <table class="min-w-max w-full table-auto">
                    ...
                    <tbody class="text-gray-600 text-sm font-light">
                        @foreach ($fermas as $ferma)
                            @include('amrani.pages.ferma.partials.tr', ['ferma'=>$ferma])
                        @endforeach
                    </tbody>
                </table>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('.destroy_ferma').on('click', function(e){
            e.preventDefault();
            var that = $(this);
            Swal.fire({
                title: 'Supprimer?',
        ...
        $('#req_submit').on('click', function(){
            ...
            if($('#ferma_service_id').val() != "-1"){
                data.ferma_service_id = $('#ferma_service_id').val();
            }
            if($('#ferma_situation').val() != "-1"){
                data.ferma_situation = $('#ferma_situation').val();
            }
